...
def notify_buyer_shipment_update(doc, method):
    # Get Contact doc from delivery_contact_name
    ...
    # Email content
    subject = f"Shipment Update for Order: {doc.name}"
    message = f"""
        <h3>Shipment Details</h3>
        <p>Dear Customer,</p>
        <p>Your shipment with ID <strong>{doc.name}</strong> is currently in status: <strong>{doc.status}</strong>.</p>
        <ul>
            ...
            <li><strong>Value of Goods:</strong> {doc.value_of_goods}</li>
